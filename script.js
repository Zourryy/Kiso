...
function startExam() {
    ...
    if(timerMin && timerMin > 0) {
        ...
        timerInterval = setInterval(() => { seconds--; let m = Math.floor(seconds / 60); let s = seconds % 60; timerDisplay.innerText = `${m}:${s < 10 ? '0' : ''}${s}`; if(seconds <= 0) { clearInterval(timerInterval); alert("Waktu Ujian Habis!"); finishExam(); } }, 1000);
...
function loadQuestion() {
    ...
    document.getElementById('exam-progress').innerText = `Soal: ${currentQIndex + 1} / ${examQuestions.length}`; document.getElementById('exam-score').innerText = `Skor: ${score}`;
    ...
    options.sort(() => 0.5 - Math.random()); let optsDiv = document.getElementById('mc-opts'); optsDiv.innerHTML = '';
    options.forEach(opt => { let btn = document.createElement('button'); btn.className = 'mc-btn'; btn.innerText = opt.text; btn.onclick = () => handleAnswer(btn, opt.isCorrect); optsDiv.appendChild(btn); });
}
function handleAnswer(btn, isCorrect) {
    ...
    if (isCorrect) { btn.classList.add('correct', 'anim-pop'); score++; document.getElementById('exam-score').innerText = `Skor: ${score}`; } 
    else { btn.classList.add('wrong', 'anim-shake'); if (showAnswer) { buttons.forEach(b => { let q = examQuestions[currentQIndex]; if(b.innerText.includes(q.arti) || b.innerText.includes(q.hiragana)) b.classList.add('correct-reveal'); }); } }
