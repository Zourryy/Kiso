...
function switchTab(tabId) {
 ...
 if(window.event && window.event.currentTarget && window.event.currentTarget.classList && !window.event.currentTarget.classList.contains('fab')) { 
...
async function loadData() {
 try {
     ...
     if (errDiv) {
         ...
         errDiv.innerHTML = `Gagal sinkron file TXT.<br><span style="font-size:12px; font-weight:normal; color:var(--text-muted);">(Info: ${err.message}. Harap jalankan index.html menggunakan Local Server / Live Server).</span>`;
     }
     loadSettings();
     renderSavedKotoba();
 }
...
// --- 11. FUNGSI COPY CLIPBOARD ---
function copyRekening() {
   navigator.clipboard.writeText("107397547525").then(() => {
       alert("Nomor Rekening Jago berhasil disalin ke clipboard!");
   }).catch(err => {
       alert("Gagal menyalin nomor rekening.");
   });
}

// 12. SERVICE WORKER RE-REGISTRATION CACHE CLEANER
